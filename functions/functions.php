...
function getValue($value_name, $data_type = "int", $method = "GET", $default_value = 0, $advance = 0, $wysiwyg = 0){
	...
	//$string_value = ($wysiwyg == 1) ? $value : trim(str_replace('"','&quot;',strval($value)));
	$valueArray	= array("int" => myIntval($value), "str" => $string_value, "flo" => floatval($value), "dbl" => doubleval($value), "arr" => $value);
	...
	return (myIntval($value));
}
/**
 * Hàm fix lỗi intval cho php
 */
...
function generateBreadcrumbs($iCat, $module="tintuc", $type=1){
	...
	if($iCat > 0){
		...
		$array_parent	= getAllParent_v2("categories_multi", "cat_id", "cat_parent_id", "cat_name", $iCat);
		if(count($array_parent) > 0){
			...
			foreach($array_parent as $key => $value){
				$link	= createlink($module, array("iCat" => $key, "iData" => $key, "nTitle" => $value['cat_name']));
				$class	= "";
				if($iCat == $key) $class	= "current";
				$nav		.= '<span class="icon_right"></span><span class="' . $class . '" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope=""><a title="' . htmlspecialbo($value['cat_name']) . '" itemprop="url" href="' . $link . '"><span itemprop="title">' . $value['cat_name'] . '</span></a></span>';
			}
		...
		switch ($module) {
			case 'news_cat':
				$nav		.= '<span class="icon_right"></span><span class="current" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope=""><a title="Tin tức" itemprop="url" href="/news/"><span itemprop="title">Tin tức</span></a></span>';
				break;

			default:
				$nav		.= '<span class="icon_right"></span><span class="current" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope=""><a title="Sản phẩm" itemprop="url" href="/home/"><span itemprop="title">Sản phẩm</span></a></span>';
				break;
		}
	...
	if($type == 0) $nav = $nav;
	else $nav	= '<div class="nav_detail breadcrumbs" itemprop="breadcrumb">' . $nav . '</div>';
