...
function handleClick(event, container, options) {
  ...
  var defaults = {
    ...
    iData:$(link).attr('iData'),
    target: link,
    clickedElement: $(link), // DEPRECATED: use target
    fragment: null,
    appendData:false, //myedit
    scrollTop:false //myedit
  }
...
function pjax(options) {
  ...
  //myedit
  //khai bao lai thong tin post ajax
  options.type	= "POST";
  var append = (options.appendData) ? 1 : 0;
  if(options.container == "detail_center"){
  		//showOverlay();
  }
  options.data = {"_pjax" : 1,append:append,"iData" : options.iData};
