...
// search type
if ($search_type != 0) {
    $sqlWhere .= " AND order_status IN ( " . convert_list_to_list_id($search_type) . ")";
}

// Search theo ID
if ($search_id > 0) $sqlWhere .= " AND order_id = " . $search_id;

// Search từ khóa
if ($search_keyword != "") {
    $sqlWhere .= " AND order_name LIKE '%" . $search_keyword . "%'";
}
...
<body>
<legend>Danh Sách Order Đồ Cũ</legend>
<div class="search">
            ...
            <tr>
                ...
                <td>
                    <select class="form-control form-control-search form-control-sm" id="search_type" name="search_type"
                        ...
                        <?php foreach ($arrOrderStatus as $key => $value) { ?>
...
<table class="table table-condensed table-hover table-bordered">
    ...
    while ($row = mysql_fetch_assoc($db_listing->result)) {
        ...
        <tr>
            ...
            <td><?= $arrOrderStatus[$row['order_status']] ?></td>
            <td>
                <a style="font-size: 14px;"
                   href="edit.php?record_id=<?= $row['order_id'] ?>&url=<?= base64_encode($_SERVER['REQUEST_URI']) ?>"><img
