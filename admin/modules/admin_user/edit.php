					...
					<form ACTION="<?=$ff_action;?>" METHOD="POST" name="edit_user">
						<table align="center" cellpadding="4" cellspacing="0" border="0">
							...
							<tr <?=$fs_change_bg?>>
								...
								<td class="textBold">
									<input type="checkbox" name="adm_all_category" onclick="checkshowlistcat()" id="adm_all_category"  <? if($edit["adm_all_category"]==1) echo ' checked="checked"';?> value="1">
									...
									<td class="textBold">
										...
										<ul>
											<?
												...
												?>
												<li><input type="checkbox" name="arelate_select[]" <? if(strpos($edit["adm_access_category"],"[" . $cat["cat_id"]  . "]")!==false) echo ' checked="checked"'?> value="<?=$cat["cat_id"]?>" /> <?=$cat["cat_name"]?></li>
												<?
												}
											?>
										</ul>
