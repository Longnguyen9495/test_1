...
$myform->add("db_categories_id", "db_categories_id", 0, 0, "", 1, "Bạn chưa chọn hình thức", 0, "");
...
<p align="center" style="padding-left:10px;">
    ...
    <tr>
        ...
        <td>
            <select name="db_categories_id" id="db_categories_id" class="form-control">
                <option value="">--[ <?= translate_text("Chọn loại danh mục") ?> ]--</option>
                <? foreach ($menu_parent as $key => $value) { ?>
                    <option value="<?= $value['db_id'] ?>"><?= $value['db_categories_name'] ?></option>
                <? } ?>
            </select>
        </td>
    </tr>
...
<script type="text/javascript">
    $("#db_name").blur(function () {
        ...
        $.post("xuly.php", {db_name: db_name}, function (data) {
            if (data == 0) {
            } else {
    ...
    function loadDistrict(div_city, div_district, current) {
        ...
        $("#" + div_district).html('<option value="0">Giong</option>');
